...
Мы реализовали загрузчик на `aya` (`EbpfLoader`). Чтобы проверить работу без изменения основного `main.rs`, вы можете написать простую утилиту, либо воспользоваться встроенными средствами ядра `iproute2` (хотя `aya` предпочтительнее).

Для `aya` мы можем запустить интеграционный тест или простой бинарник. 
